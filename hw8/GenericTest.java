...
import static info.kgeorgiy.courses.java_intro.homeworks.hw8.Util.*;
        ...
        for (int x = x1; x <= x2; x++) {
            for (int y = y1; y <= y2; y++) {
                for (int z = z1; z <= z2; z++) {
                    ...
                    assert expected == null ? actual == null : expected.equals(actual) : String.format("table[%d][%d][%d] = %s expected %s", x - x1, y - y1, z - z1, actual, expected);
                }
            }
        }
    }
